...
def main():
    ...
    logger.info("------------------ DDPG ---------------------")
    logger.info("Env: ContinuousCartPole")

    env = ContinuousCartPoleEnv()

    obs_dim = env.observation_space.shape[0]
    act_dim = env.action_space.shape[0]
    logger.info('Env Obs_dim: {}, Env Action_dim: {}'.format(obs_dim, act_dim))
    ...
    # 加载模型并评估
    ''' 
    if os.path.exists('./DDPG_model/model.ckpt'):
        agent.restore('./DDPG_model/model.ckpt')
    ...
    while episode < args.train_total_episode:
        for i in range(100):
            ...
            summary.add_scalar('train/episode_reward', total_reward, episode)
            episode += 1

        eval_reward = run_evaluate_episodes(agent, env, render=False)
        summary.add_scalar('eval/episode_reward', eval_reward, episode)
        logger.info('episode:{}    Test reward:{}'.format(
            episode, eval_reward))

    save_path = './DDPG_model/model.ckpt'
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        default=int(6e3),
        type=int,
        help='Max time episode to run environment')
